    ...
    self._sampleFeeLevels(opts.coin, opts.network, samplePoints(), function(err, feeSamples) {
      var values = _.map(feeLevels, function(level) {
        ...
        if (err) {
          ...
          // Add 1000 duffs (MIN_FEE_PER_KB) if estimated fee is beneath network minimum
          if (result.feePerKb < Defaults.MIN_FEE_PER_KB) result.feePerKb += Defaults.MIN_FEE_PER_KB;
 ...
 * @param {string} [opts.network = 'livenet'] - The Lifetioncoin network for this transaction.
