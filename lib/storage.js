...
Storage.prototype.connect = function(opts, cb) {
  ...
  this.db.addCollection(collections.WALLETS);
  this.db.addCollection(collections.TXS);
  this.db.addCollection(collections.ADDRESSES);
  this.db.addCollection(collections.NOTIFICATIONS);
  this.db.addCollection(collections.COPAYERS_LOOKUP);
  this.db.addCollection(collections.PREFERENCES);
  this.db.addCollection(collections.EMAIL_QUEUE);
  this.db.addCollection(collections.CACHE);
  this.db.addCollection(collections.FIAT_RATES);

  console.log('Connection established to minimongo');
  return cb();
...
Storage.prototype.storeAddressAndWallet = function(wallet, addresses, cb) {
  ...
  async.filter(addresses, function(address, next) {
    ...
    // TODO assuming this array always contains just one item...

    self.db[collections.ADDRESSES].findOne({
      address: newAddresses[0].address,
    }, {}, function(res) {
      if(res) newAddresses[0]._id = res._id;
